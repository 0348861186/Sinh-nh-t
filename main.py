...
# ============================================================
# BỘ TỪ ĐIỂN CHUYÊN NGÀNH NHÀ MÁY & QUẢN LÝ FILE JSON
# ============================================================
...
def load_glossary():
    if GLOSSARY_FILE.exists():
        try:
            ...
            pass
    return DEFAULT_GLOSSARY_ZH_VI.copy()

def save_glossary(glossary_dict):
    try:
        with open(GLOSSARY_FILE, "w", encoding="utf-8") as f:
            json.dump(glossary_dict, f, ensure_ascii=False, indent=4)
    except Exception:
        pass
...
def apply_glossary(text, mode):
    ...
    if mode == "Trung ➔ Việt":
        ...
        if text_cleaned != text.strip():
            return text_cleaned
    else:
        vi_zh = {v: k for k, v in glossary.items()}
        if text_cleaned in vi_zh:
            return vi_zh[text_cleaned]
        for v, k in vi_zh.items():
            if v in text_cleaned:
                text_cleaned = text_cleaned.replace(v, k)
        if text_cleaned != text.strip():
            return text_cleaned
...
with st.sidebar.expander("⚙️ Bổ sung từ điển nhanh"):
    ...
    if st.button("Thêm vào từ điển"):
        if new_zh and new_vi:
            st.session_state["glossary"][new_zh] = new_vi
            save_glossary(st.session_state["glossary"])
            st.success(f"Đã thêm: {new_zh} ➔ {new_vi}")
        else:
            st.warning("Vui lòng điền đủ cả hai ô.")
